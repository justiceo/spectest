...
export class PluginHost {
  ...
  public async dispatchRunStart(tests: TestCase[]): Promise<void> {
    for (const cb of this.onRunStartCbs) await cb(tests);
  }

  public async dispatchRunEnd(result: TestRunResult): Promise<void> {
    for (const cb of this.onRunEndCbs) await cb(result);
  }

  public async dispatchTestStart(test: TestCase): Promise<void> {
    for (const cb of this.onTestStartCbs) await cb(test);
  }

  public async dispatchTestEnd(test: TestCase, result: TestResult): Promise<void> {
    for (const cb of this.onTestEndCbs) await cb(test, result);
  }
}
